...
- 新增 Claude Code 原生 Manifest 与 Marketplace；保留 Codex 原生文件。两个 Catalog 共同指向 `plugins/ad-wiki/`。
...
- Requirements: R1, R4-R8, R10, R13
- Dependencies and accepted-design pointers: Technical Design sections 3-5, 8-9
- Affected modules and mutation: `.claude-plugin/marketplace.json`, `plugins/ad-wiki/.claude-plugin/plugin.json`, Codex Manifest version, Runtime `PLUGIN_VERSION`,模板与版本说明
...
- Affected modules and mutation: `plugins/ad-wiki/skills/ad-wiki-maintainer/SKILL.md`、workflow reference、packaging tests
...
- Affected modules and mutation: `plugins/ad-wiki/tests/test_packaging.py`，必要的测试 fixture 或无持久化验证脚本
...
- Cross-unit evidence, required: `python3 -m unittest discover -s plugins/ad-wiki/tests -v`、`python3 -m compileall -q plugins/ad-wiki/scripts`、临时复制后的 Runtime help/init/validate。
...
- 仓库只有一份 canonical Maintainer Skill、references/assets 和 Runtime。
