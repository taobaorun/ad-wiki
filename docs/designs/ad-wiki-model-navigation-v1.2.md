...
Authority: 用户于 2026-08-20 明确要求完全删除 `build_query_context`，不保留兼容；千页以内信任模型通过 Wiki 与本地文本搜索自主检索，超过约 1000 页后再基于证据考虑 BM25。

## 决策摘要

- Query 与 Maintainer 直接读取 `ad-wiki.yaml`、Bundle 索引和完整 Markdown 页面。
- 模型使用 `rg` 或宿主等价能力在明确 Bundle 内搜索，可迭代关键词、同义词、标识符和链接。
...
question
  ...
  → rg within bundle_root
  → read full relevant Concepts
  → refine search or follow Markdown links when needed
  → optional registered-Raw fallback for one narrow missing detail
...
- OKF 保持 `0.2`，AD-Wiki Profile 保持 `0.1`；Plugin 发布为 `1.2.0`。
...
- 行为：新上下文 Agent 能通过 index + Bundle 内文本搜索回答代表性问题；Query 字节只读；Maintainer 检查 staged diff，不写 host memory，不把 partial 报告成完成。
