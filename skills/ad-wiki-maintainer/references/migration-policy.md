...
The `1.5.0` Plugin intentionally keeps AD-Wiki Profile `0.1`. Optional structural code-index artifacts and bindings remain rebuildable local state outside the Bundle; Companions remain ordinary OKF Concepts, so no knowledge-data migration is required. Existing repositories receive no automatic code scan, dependency bootstrap, or structural refresh. Model-only Code Wiki remains available; structural mode is explicit. The migration command returns `status: current` for Profile `0.1` and refuses every unregistered path. A future data release must add and test an exact source-to-target transform before advertising that migration.
