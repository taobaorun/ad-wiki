...
The `1.6.0` Plugin intentionally keeps AD-Wiki Profile `0.1`. Query Primary-Source descent and read-only Health Report/Assessment v1 change no required Bundle data. Optional structural code-index artifacts, bindings and health evidence remain rebuildable or caller-owned state outside the Bundle; Companions, key-system inventories and Glossaries remain ordinary OKF Concepts, so no knowledge-data migration is required. Existing repositories receive no automatic code scan, health assessment, dependency bootstrap or structural refresh. Model-only Code Wiki remains available; structural mode is explicit. The migration command returns `status: current` for Profile `0.1` and refuses every unregistered path. A future data release must add and test an exact source-to-target transform before advertising that migration.
