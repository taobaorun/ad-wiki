...
The `1.9.0` Plugin intentionally keeps AD-Wiki Profile `0.1`. Reviewed medium/high-risk Writeback, Query-side ephemeral candidates, portable code-source registry v1, exact private worktree bindings, Query Primary-Source descent, read-only Health Report/Assessment v1, and standalone read-only directory/ZIP Skill delivery change no required Bundle data. Optional structural code-index artifacts, private bindings and health evidence remain rebuildable or caller-owned state outside the Bundle; the portable code-source registry is created lazily or rebuilt from validated Code Wiki runs. Companions, key-system inventories and Glossaries remain ordinary OKF Concepts, so no knowledge-data migration is required. Existing repositories receive no automatic code scan, worktree discovery, health assessment, dependency bootstrap, structural refresh or delivery build. Model-only Code Wiki remains available; structural mode is explicit. The migration command returns `status: current` for Profile `0.1` and refuses every unregistered path. A future data release must add and test an exact source-to-target transform before advertising that migration.
